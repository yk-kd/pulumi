...
var (
	stackNameStartRegex = regexp.MustCompile("^[A-Za-z0-9]")
	stackNameContRegex  = regexp.MustCompile("^[A-Za-z0-9_.-]*")
)
...
func ParseStackName(s string) (StackName, error) {
	...
	// We use two regexes to validate the stack name. The first checks the first character, the second checks
	// the rest. This is because the first character has a different set of valid characters to the rest.
	if !stackNameStartRegex.MatchString(s) {
		return StackName{}, fmt.Errorf(
			"a stack name must start with only alphanumeric: "+
				"invalid character %q", s[0])
	}
	failure := -1
	// Check the rest
	if match := stackNameContRegex.FindStringIndex(s[1:]); match == nil {
		// We have failed to find any match, so the first char must be invalid.
		failure = 1
	} else if match[1] != len(s)-1 {
