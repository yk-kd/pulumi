...
/**
 * monitorSupportsCallbacks returns a promise that when resolved tells you if the resource monitor we are
 * connected to is able to support callbacks across its RPC interface.
 *
 * @internal
 */
export async function monitorSupportsCallbacks(): Promise<boolean> {
    return monitorSupportsFeature("callbacks");
}
