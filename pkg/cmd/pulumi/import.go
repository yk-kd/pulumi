...
func parseImportFile(f importFile, stack tokens.Name, proj tokens.PackageName, protectResources bool) ([]deploy.Import, importer.NameTable, error) {
	// First check for uniqueness and ambiguity, takenNames tracks both that a name is used (it's in the map) and if it's ambiguous (it's true).
	...
	// A mapping from name to URN, prefilled with emptys and what was in the name table so we can do existence checks for expected names
